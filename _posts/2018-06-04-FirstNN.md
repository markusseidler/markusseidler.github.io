...
#Defining the relevant variables and external inputs.
    ...
    '''
    This function manages the so-called Forward propagation.
    Calculating the output step by step.
    The dot product the first layer (input layer X) and weights matrix W1 results
    ...
    in creating an error. Mathematically, we take the partial derivatives layer by layer
...
This includes quite a lot of concepts. 
